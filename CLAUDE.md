...
Props passed to `DashboardContent`: `initialTransactions`, `initialTotalIncome`, `initialTotalExpenses`, `initialCategoryData`, `initialDailyData`, `initialTopCategory`, `initialRecurring`, `initialBudgets`, `initialCategories`, `month`, `year`, `prevTotalExpenses`, `prevTotalIncome`, `prevCategoryData`, `prevDailyData` (prev period's daily buckets — consumed by `PaceChart`; empty array in the all-time view), `initialWealthData` (ledger-basis monthly buckets for `WealthCurve` — populated only in the all-time view, empty array otherwise).
