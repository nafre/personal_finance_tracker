...
### ~~5i — Reject Malformed Numeric Tokens Instead of Truncating~~ DONE
`extractNumericValue`'s legacy fallback used `parseFloat` + `isNaN`, which silently truncates trailing garbage instead of rejecting the token (`parseFloat("2-in-1") === 2`, `parseFloat("2k") === 2`). A mixed alphanumeric token could hijack amount detection and shove the real amount into the note — `"buy 2-in-1 pack 15"` used to parse as amount `2`, note `"pack 15"` instead of amount `15`. Fixed by requiring the whole cleaned token to match `^\d+(\.\d+)?$` before accepting it, so a malformed token now falls through to the next real numeric token (or fails the parse entirely, rather than silently recording a wrong amount). Bundled a related one-line fix in the same pass: repeated `#label` tokens are now deduped (`"coffee 4.5 #work #work"` used to produce `labels: ["work", "work"]`). Covered by new `lib/parser.test.ts` cases.
